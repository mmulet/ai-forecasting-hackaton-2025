...
def score_sample():
    async def score(state: TaskState, target: Target) -> Score:
        ...
        if state.metadata["answer_format"] == "MC":
            mc_answer = answer[:2] # should be A. or B. etc
            return Score(value="C" if mc_answer in target.text else "I",)
...
def evaluate_model_on_adele() -> Task:
    ...
    ds = ds.filter(lambda x: x.metadata["answer_format"] == "MC")  # limit Multiple Choice for faster testing
   
    
    return Task(
        dataset=ds,
        solver=mas_agent("mas_configs/verifier.yaml"),
        scorer=[score_sample()]
